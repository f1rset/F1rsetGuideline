...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    fit = function(X, y) {
                        ...
                        # Count word occurrences in the training set (P(feature_i | class))
                        word_counts <- X %>%
                        bind_cols(y) %>%
                        group_by(y) %>%
                        count(splitted) %>%
                        spread(y, n, fill = 0)
                        # Compute likelihoods and store in the class field for future use
                        self$priors <- priors
...
predicted_labels <- sapply(tidy_test$splitted, model$predict)
actual_labels <- tidy_test$author
